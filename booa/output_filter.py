...
# OWS is BOOA-specific; generic keys are handled by Nous's agent.redact — but the
# console log stream runs this filter alone (no agent.redact), so cover the common
# provider-key prefixes here too.
_API_KEY_PATTERNS: list[tuple[re.Pattern[str], str]] = [
    ...
    (re.compile(r"\bsk-[A-Za-z0-9_\-]{20,}\b"), "provider"),
]
